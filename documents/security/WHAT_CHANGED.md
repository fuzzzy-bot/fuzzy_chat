...
`feat/message-length-manager`) with the hardened app at `df7dab6` on `agent/chat-harden-rust-crypto-core` · **Owner
decision that asked for this:** D-10 (`flow/fuzzy-chat-hardening/OWNER_DECISIONS.md`).
...
product consequence needed your decision and got it: readable history stays on the device, sealed at rest under a
per-chat key (D-1).
...
flowchart LR
  ...
  subgraph RUST[Rust crate fuzzy_crypto_core - the only key holder]
    ...
    RC[RustCrypto: chacha20poly1305, aead-stream, argon2, hkdf, sha2, zeroize, getrandom]
    API --> CORE
    CORE --> STORE
    STORE --> STATE
    API --> PAIRING
  ...
  subgraph DISK[On this device only]
    ...
    ISAR[Isar StoredMessageData.sealedPlaintext - 0x20 local seal per message]
...
| History | Isar `StoredMessageData` kept only `encryptedMessage`; `MessageData.fromStored` set `decryptedMessage = ''` and `connected_chat_cubit.dart:106-120` re-decrypted every text row on page load | The plaintext is sealed once (`0x20` local seal) into the new `StoredMessageData.sealedPlaintext` column and opened on read; nothing is re-decrypted ([`message_data_repository.dart`](../../lib/src/fuzzy_chat/data/repositories/message_data_repository/message_data_repository.dart), [`PROTOCOL.md` §10.4](PROTOCOL.md#104-local-seal-of-message-history)) | The ratchet makes re-decryption impossible; a sender cannot decrypt its own output either | Chat history looks the same; it loads without a decrypt per row |
| History key | — | Today: `HKDF(store key, "fuzzy-local-seal-v1")`, one per device. **D-1 (2026-09-13): one key per chat**, derived from that chat's own key material and destroyed with the chat — feature F2-12, in flight at this commit (`flow/fuzzy-chat-hardening/features/F2-12.md`) | Your D-1 answer: defence in depth, app-lock password stays the outer gate | Not visible |
...
| Key derivation | HKDF-SHA256 per message with `fuzz_v1` info (good — carried forward as the ratchet's job) | HKDF-SHA256 used once, store key → local key (`fuzzy-local-seal-v1`); per-message derivation is Olm's | The ratchet subsumes the per-message subkey |
...
| Hash / KDF | PointyCastle `SHA256Digest`, `HKDFKeyDerivator`; `package:crypto` 3.0.7 (transitive, imported directly by Basics) for SHA-256 | `sha2` **0.11.0**, `hkdf` **0.13.0** |
...
| Pinning | `pubspec.lock` | `pubspec.lock` + `Cargo.lock`, CI builds `--locked`; the five crypto/FFI crates `=`-pinned in `Cargo.toml`; 20 byte-exact vectors fail `cargo test` if any dependency changes a byte ([`PROTOCOL.md` §15](PROTOCOL.md#15-reference-crates-and-versions)) |
...
| A test tested a different KDF than shipped | `test/src/core/utils/aes_manager/ephemeral_key_derivation_test.dart:11-29` reimplemented HKDF with `salt: null, info: nonce`; production used `salt: salt, info: fuzzVersionInfo` | Replaced by 20 byte-exact vectors generated through the production code |
...
| **store key** | The one 32-byte random key per installation. It seals every chat's `.state` file and (through HKDF) the history. Exists on disk only *wrapped* under the app-lock password (or the empty string when the lock is off). | [`PROTOCOL.md` §10.2](PROTOCOL.md#102-store-key-lifecycle) |
| **history key** | The key that seals the readable copy of messages in the database. At the commit this document describes it is the **local key** (`HKDF(store key, "fuzzy-local-seal-v1")`, one per device). Owner decision D-1 changes it to **one per chat**, derived from that chat's own key material — that is feature F2-12, in flight when this was written. | [`PROTOCOL.md` §10.4](PROTOCOL.md#104-local-seal-of-message-history); HQ `flow/fuzzy-chat-hardening/features/F2-12.md` |
| **local seal** | A `0x20` blob: XChaCha20-Poly1305 of a piece of local data (a message's plaintext, a vault item, a state file) under a local key. Never travels; the paste path refuses it. | [`PROTOCOL.md` §6.9](PROTOCOL.md#69-local-seal-0x20-storage-only) |
...
| **HKDF** | A standard way to derive a sub-key from a key with a label. Used once: store key → local key with the label `fuzzy-local-seal-v1`. | [`PROTOCOL.md` §3](PROTOCOL.md#3-key-material-per-chat) |
...
| **SBOM** | Software Bill of Materials: the CycloneDX lists of every Rust crate (175) and every Dart package (203) in the build, regenerated in CI and diffed against the lock files. | [`sbom/`](sbom/) |
...
|---|---|---|---|
| **D-1** | Blobs are now single-use (forward secrecy). Keep a readable copy of history on the device, or show "unfuzzed once" placeholders? | **Keep readable history, sealed at rest — under a key per chat**, derived from that chat's own key material, with the app-lock password as the outer gate. | Sent and received plaintext are sealed into `StoredMessageData.sealedPlaintext` and shown from there; a blob pasted twice says "already unfuzzed". The per-chat key is feature F2-12 (in flight at this commit; today's seal uses the per-device local key — [`PROTOCOL.md` §10.4](PROTOCOL.md#104-local-seal-of-message-history)). Deleting a chat destroys its history key. Unblocks the archive export (F2-10) and the on-screen trade-off copy (F2-11). |
...
| **D-3** | This Mac has no Apple Development certificate; the Xcode project is manual-signed for team `C9387PQ63V`. | **Codemagic** will sign macOS releases; nothing needed on the build Mac. | CI's macOS artifact stays unsigned/un-notarized; QA builds run with Flutter's code-signing passthrough env. |
...
| **D-7** | "Copy as link" for a message carries the chat id (`c=`) in the clear; biometric unlock keeps the app-lock password in the OS keystore; the chat store never auto-locks (pre-existing design). | **Keep the chat id in message links**; state all three facts in the trade-off copy. | No code change; wording for F2-11 and `THREAT_MODEL.md` R12/R20/R21. |
...
| **D-10** | `v1.0.0` is already a public tag (2025-05-19, old stack). Ship as `v1.1.0` or move the tag? | **`v1.1.0`.** Plus this document. | The hardened build ships as `v1.1.0`; the 2025 tag stays where it is. Merging the branch also brings in the 39-commit UI-kit migration branch (`feat/message-length-manager`) it was built on. |
...
| Tests | 27 pure-Dart crypto tests (deleted with the stack) | `cargo test --locked` **161**; `fvm flutter test` **240** (233 at the sweep) | `log/F5-5.md`; sweep `logs/{cargo_test,flutter_test}.txt` |
| Byte-exact test vectors | none | 20 vectors, 45 files, checked on every `cargo test` | [`vectors/README.md`](vectors/README.md) |
